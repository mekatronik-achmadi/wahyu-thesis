...
static void cmd_motortest(BaseSequentialStream *chp, int argc, char *argv[]) {
    ...
    motor_Run(MOTOR2,MOTOR2_CW,300,500);
}
...
static void cmd_motorrun(BaseSequentialStream *chp, int argc, char *argv[]) {
    ...
    if(argc != 4){chprintf(chp,"Usage: morun <number> <direction> <steps> <speedinterval>\r\n");return;}
    motnum = atoi(argv[0]);
    motdir = atoi(argv[1]);
    motstep = atoi(argv[2]);
    motspd = atoi(argv[3]);

    chprintf(chp,"Run on Motor %i at %i dir and %i steps on %i interval\r\n",motnum,motdir,motstep,motspd);
    motor_Run(motnum,motdir,motstep,motspd);
}
